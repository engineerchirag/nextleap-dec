/*
1. variable (API, User Interaction)
...
interface ICompareProductDetail {
  title: string,
  img: string
}
...
"use strict"
var cart: string[] = []
let compareProductList: ICompareProductDetail[] = []
let compareWrapperTimeoutId
fetch("https://sandbox.nextleap.app/products/fetch")
  .then(function(response) {
    return response.json()
  })
  .then(function(data: IResponse) {
    removeLoader()
    renderHeader()
...
function renderProductList(cards: IProductCard[]) {
  for (var i in cards) {
    renderSingleProduct(cards[i])
  }
}
function renderSingleProduct(product: IProductCard) {
  ...
  productReviewRatngPara.append(
    productStarRatingSpan,
    " "
      .concat(
        product.ratings.totalRatingsNum.toLocaleString("en-US"),
        " Ratings & "
      )
      .concat(
        product.ratings.totalReviewsNum.toLocaleString("en-US"),
        " Reviews"
      )
  )
  ...
  productPrice.innerHTML = "&#8377; ".concat(
    product.price.finalPrice.toLocaleString("en-US")
  )
  var productMRP = document.createElement("s")
  productMRP.style.marginTop = "10px"
  productMRP.style.color = "#878787"
  productMRP.innerHTML = "&#8377; ".concat(
    product.price.mrp.toLocaleString("en-US")
  )
  ...
  exchangeOfferPrice.innerHTML = "&#8377; ".concat(
    product.exchangeOfferDiscount.data.toLocaleString("en-US")
  )
  ...
  addToCompareInput.id = "add-to-compare-input-"
    .concat(product.title, "-")
    .concat(product.image.url)
...
function compareClick(title, img) {
  var newCompareProductList: ICompareProductDetail[] = [];
  var alreadyPresent = false
  compareProductList.map(function(value: ICompareProductDetail) {
...
function renderCompareDiv() {
  ...
  compareButton.onmouseenter = function() {
    ...
    if (compare) {
      compare.style.transform = "scale(1)"
    }
  }
...
function removeAllCompare() {
  compareProductList.map(function(value) {
    var input: HTMLInputElement | null = document.getElementById(
      "add-to-compare-input-".concat(value.title, "-").concat(value.img)
    ) as HTMLInputElement;
    if (input) {
      input.checked = false
    }
...
function compareStateChange(alreadyPresent) {
  ...
  if (compareProductList.length == 0) {
    ...
    if (compareWrapper) {
      compareWrapper.style.visibility = "hidden"
    }
  } else {
    if (compareButton) {
    ...
    if (compareWrapper) {
      compareWrapper.style.visibility = "visible"
    }
    if (
      (compareProductList.length == 1 && !alreadyPresent) ||
      (compareWrapper && compareWrapper.style.transform == "scale(1)")
    ) {
      if (compareWrapper) {
        compareWrapper.style.transform = "scale(1)"
      }
      compareWrapperTimeoutId = setTimeout(function() {
        if (compareWrapper) {
          compareWrapper.style.transform = "scale(0)"
    ...
    if (compareCount) {
      compareCount.innerHTML = compareProductList.length.toString()
    }
      ...
      crossButton.onclick = function() {
        var input = document.getElementById(
          "add-to-compare-input-"
            .concat(compareProductList[i].title, "-")
            .concat(compareProductList[i].img)
        )
        input.checked = false
        var alreadyPresent = compareClick(
          compareProductList[i].title,
          compareProductList[i].img
        )
        compareStateChange(alreadyPresent)
      }
      singleCompareProduct.append(
        singleCompareProductImg,
        singleCompareProductTitle,
        crossButton
      )
      if (productList) {
...
function renderFooter(footerData) {
  ...
  var _loop_1 = function() {
    ...
    if (columns[i].dataType == "ARRAY") {
      ...
      columns[i].dataType == "MAIL_ADDRESS" ||
